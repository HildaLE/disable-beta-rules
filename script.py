...
def fetch_ruleset():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Error fe atching ruleset: {response.status_code}")
            print(response.text)
...
if __name__ == "__main__":
    fetch_ruleset()  # Fetch and append the current ruleset to file
    clean_file("rules.json")  # Clean the rules.json file
    extract_beta_ids("rules.json", "beta_ids.txt")  # Extract beta rule IDs
    disable_rules("beta_ids.txt")  # Disable the extracted rules
